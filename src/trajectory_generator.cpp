...
void plannerCb(const trajectory_msgs::MultiDOFJointTrajectory& msg)
{
	...
	for (int i = 0; i <= n; ++i)
	{
		...
		waypoints(i, 2) = msg.points[i].transforms[0].translation.z+1;
	}
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			Eigen::MatrixXf position = poly_diff(8, 0, scale/20.0)*coefficients.block(8*i, 0, 8, 3);
			...
			marker.pose.position.z = position(2)-1;
		...
		// std::cout << "trajectory: " << poly_diff(8, 0, 1)*coefficients.block(8*i, 0, 8, 3) << std::endl;
	}

	// send trajectory commands
	trajCb(coefficients, waypoints);
